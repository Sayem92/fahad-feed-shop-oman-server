...
async function run() {
  try {
    ...
    const productsCollection = client.db("Omanshop").collection("products");
...
app.get("/", (req, res) => {
  res.send("Fahad feed shop server is running.............");
});

app.listen(port, () => {
  console.log("Fahad feed shop server running on:", port);
});
